...
export default function MatchScreen({ navigation, route }) {
  ...
  // ── Real data from route params (passed from HomeScreen on match) ─────────
  ...
  const theirAvatar = matchedUser.profilePicture
    ...
    || `https://ui-avatars.com/api/?name=${encodeURIComponent(matchedUser.name)}&background=FF4D6D&color=fff&size=200`;
